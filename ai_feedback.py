...
def generate_ai_feedback(
    report_content: str, developer_name: str, commit_sha: str
) -> str:
    """
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Rapport d'erreur pour le commit {commit_sha} par {developer_name}:\n\n{report_content}",
                },
            ],
            temperature=0.7,
            max_tokens=500,
...
def main():
    parser = argparse.ArgumentParser(
        description="Génère un feedback IA personnalisé pour les erreurs de qualité de code."
    )
    parser.add_argument(
        "report_file",
        help="Chemin vers le fichier contenant le rapport d'erreur combiné.",
    )
    parser.add_argument(
        "developer_name", help="Nom du développeur (ex: $GITHUB_ACTOR)."
    )
    ...
    feedback = generate_ai_feedback(
        report_content, args.developer_name, args.commit_sha
    )
