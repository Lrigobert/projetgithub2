...
def load_products_from_json() -> List[Dict[str, Any]]:
    ...
    if not os.path.exists(DATA_FILE):
        print(f"Attention: Le fichier {DATA_FILE} n'existe pas. Retourne une liste vide.")
        return []
    try:
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except json.JSONDecodeError:
        print(f"Erreur: Impossible de décoder le fichier JSON {DATA_FILE}. Retourne une liste vide.")
        return []
...
def create_product(products: List[Dict[str, Any]], new_product: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def read_product(products: List[Dict[str, Any]], product_id: str) -> Dict[str, Any] | None:
    """Récupère un produit par son ID."""
    return next((p for p in products if p['id'] == product_id), None)

def update_product(products: List[Dict[str, Any]], updated_product: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def delete_product(products: List[Dict[str, Any]], product_id: str) -> List[Dict[str, Any]]:
...
def add_to_cart(cart_items: List[Dict[str, Any]], product: Dict[str, Any], quantity: int) -> List[Dict[str, Any]]:
...
def remove_from_cart(cart_items: List[Dict[str, Any]], product_id: str) -> List[Dict[str, Any]]:
